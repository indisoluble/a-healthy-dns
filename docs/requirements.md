...
- **R23** The Docker image must run the application as the Chainguard default non-root uid `65532` while retaining the ability to bind container port `53` when deployments grant `NET_BIND_SERVICE` in the capability bounding set. The Python interpreter carries a `NET_BIND_SERVICE` file capability set at build time; deployments must not activate `no-new-privileges`, which would block that file capability.
