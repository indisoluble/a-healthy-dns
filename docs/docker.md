...
Docker is the recommended deployment method for A Healthy DNS. The official Docker image is available on [Docker Hub](https://hub.docker.com/r/indisoluble/a-healthy-dns) and includes all dependencies, security hardening, and production-ready configuration.
...
Configuration is provided via environment variables. See [docs/configuration.md](configuration.md) for detailed parameter descriptions.

### Required Variables

| Variable | Description | Example |
|----------|-------------|---------|
| `DNS_HOSTED_ZONE` | Hosted zone domain name | `example.com` |
| `DNS_ZONE_RESOLUTIONS` | Subdomain and IP configuration (JSON) | `{"www":{"ips":["192.168.1.100"],"health_port":8080}}` |
| `DNS_NAME_SERVERS` | Name servers (JSON array) | `["ns1.example.com"]` |
...
| `DNS_LOG_LEVEL` | `info` | Logging level (debug, info, warning, error) |
| `DNS_TEST_MIN_INTERVAL` | `30` | Health check interval (seconds) |
| `DNS_TEST_TIMEOUT` | `2` | Health check timeout (seconds) |
| `DNS_ALIAS_ZONES` | `[]` | Additional zones (JSON array) |
| `DNS_PRIV_KEY_PATH` | — | Path to DNSSEC private key |
| `DNS_PRIV_KEY_ALG` | `RSASHA256` | DNSSEC algorithm |
...
```bash
# Check exit code
docker ps -a | grep a-healthy-dns
...
# 3. Add capability
docker run -d --cap-add=NET_BIND_SERVICE -p 53:53/udp ...
```
...
docker inspect a-healthy-dns | jq '.[0].HostConfig.NetworkMode'
...
# Set memory limit
docker update --memory="256m" a-healthy-dns

# Or recreate with limit
docker stop a-healthy-dns
docker rm a-healthy-dns
docker run -d --memory="256m" ... indisoluble/a-healthy-dns
```
